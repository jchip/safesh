...
/**
 * ShellArray - An array wrapper with ShellString properties
 *
 * Used for commands that return arrays (like ls, find, etc.)
 */
export class ShellArray<T> extends Array<T> {
  ...
  constructor(
    ...
    super(...items);
    this.stdout = items.map(String).join("\n");
    if (items.length > 0) {
      this.stdout += "\n";
    }
    this.stderr = stderr;
   ...
   * Write to file
   */
  async to(file: string): Promise<ShellArray<T>> {
    await fs.write(file, this.stdout, this._options);
    return this;
  }
   ...
   * Append to file
   */
  async toEnd(file: string): Promise<ShellArray<T>> {
    await fs.append(file, this.stdout, this._options);
    return this;
  }
   ...
   * Create a success result
   */
  static ok<T>(items: T[], options?: SandboxOptions): ShellArray<T> {
    return new ShellArray(items, "", 0, options);
  }

  /**
   * Create an error result
   */
  static error<T>(stderr: string, code: number = 1): ShellArray<T> {
    return new ShellArray<T>([], stderr, code);
  }
}
