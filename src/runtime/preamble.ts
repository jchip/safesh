...
export function buildPreamble(
  ...
  preambleLines.push(
    ...
    `  if (cmd && typeof cmd.stream === 'function' && typeof cmd[Symbol.asyncIterator] !== 'function') {`,
    `    let __code = 1;`,
    `    let __pipeStatus: number[] | undefined;`,
    ...
    `        __pipeStatus = Array.isArray(__chunk.pipeStatus) ? __chunk.pipeStatus : undefined;`,
    `      }`,
    `    }`,
    `    return __setPipeStatus(__pipeStatus, __code);`,
    `  }`,
    `  if (cmd && typeof cmd[Symbol.asyncIterator] === 'function') {`,
    `    try {`,
    `      for await (const __line of cmd) {`,
    `        await Deno.stdout.write(__enc.encode(String(__line) + "\\n"));`,
    `      }`,
    `    } catch (__error) {`,
    `      const __message = __error instanceof Error ? __error.message : String(__error);`,
    `      await Deno.stderr.write(__enc.encode(__message + (__message.endsWith("\\n") ? "" : "\\n")));`,
    `      return __setPipeStatus(undefined, 1);`,
    `    }`,
    `    return __setPipeStatus(undefined, 0);`,
    `  }`,
    `  const result = await cmd;`,
    `  if (typeof result === 'boolean') return __setPipeStatus(undefined, result ? 0 : 1);`,
    `  if (!result) return __setPipeStatus(undefined, 1);`,
    `  if (result.output) {`,
    ...
    `    }`,
    `  }`,
    `  return __setPipeStatus(result.pipeStatus, result.code ?? 1);`,
    `}`,
    ...
    `    let __pipeStatus: number[] | undefined;`,
    `    for await (const __chunk of cmd.stream()) {`,
    `      if (__chunk.type === 'stdout' && __chunk.data) __stdout += __chunk.data;`,
    `      else if (__chunk.type === 'stderr' && __chunk.data) __stderr += __chunk.data;`,
    `      else if (__chunk.type === 'exit') {`,
    `        __code = __chunk.code ?? 1;`,
    `        __pipeStatus = Array.isArray(__chunk.pipeStatus) ? __chunk.pipeStatus : undefined;`,
    `      }`,
    `    }`,
    `    __setPipeStatus(__pipeStatus, __code);`,
    `    return { code: __code, stdout: __stdout, stderr: __stderr, success: __code === 0, pipeStatus: __pipeStatus };`,
    `  }`,
    `  if (cmd && typeof cmd[Symbol.asyncIterator] === 'function') {`,
    `    let __stdout = "";`,
    `    try {`,
    `      for await (const __line of cmd) __stdout += String(__line) + "\\n";`,
    `    } catch (__error) {`,
    `      const __message = __error instanceof Error ? __error.message : String(__error);`,
    `      const __stderr = __message + (__message.endsWith("\\n") ? "" : "\\n");`,
    `      __setPipeStatus(undefined, 1);`,
    `      return { code: 1, stdout: __stdout, stderr: __stderr, success: false };`,
    `    }`,
    `    __setPipeStatus(undefined, 0);`,
    `    return { code: 0, stdout: __stdout, stderr: "", success: true };`,
    `  }`,
    `  const result = await cmd;`,
    `  if (result && (typeof result.stream === 'function' || typeof result[Symbol.asyncIterator] === 'function')) return await __captureCmd(result);`,
    `  if (typeof result === 'boolean') {`,
    `    const __code = result ? 0 : 1;`,
    `    __setPipeStatus(undefined, __code);`,
    `    return { code: __code, stdout: "", stderr: "", success: __code === 0 };`,
    `  }`,
    `  if (!result) {`,
    `    __setPipeStatus(undefined, 1);`,
    ...
    `  __setPipeStatus(result.pipeStatus, __code);`,
    `  return { code: __code, stdout: __stdout, stderr: __stderr, success: __code === 0, pipeStatus: result.pipeStatus };`,
    `}`,
    "",
    ...
    `  const __value = await Promise.resolve(__result);`,
    `  if (__value === undefined || __value === null) return "";`,
    `  if (Array.isArray(__value)) return __value.join("\\n").replace(/\\n+$/, "");`,
    `  if (typeof (__value as any).text === "function") return (await (__value as any).text()).replace(/\\n+$/, "");`,
    `  if (typeof (__value as any).collect === "function") return (await (__value as any).collect()).join("\\n").replace(/\\n+$/, "");`,
    `  if (typeof __value === "string") return __value.replace(/\\n+$/, "");`,
    `  if (typeof (__value as any).output === "string") return (__value as any).output.replace(/\\n+$/, "");`,
    `  if (typeof (__value as any).stdout === "string") return (__value as any).stdout.replace(/\\n+$/, "");`,
